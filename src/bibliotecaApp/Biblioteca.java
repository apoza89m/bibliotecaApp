...
public class Biblioteca {
	...
	public void addPrestamo(Prestamo prestamo) {
		if (prestamo.getLibro().getEjemplosDisponibles() > 0) {
			...
			System.out.println("No existen unidades para prestar");
	}
	...
	public void imprimirTop10LibrosMasPrestados() {
		List<Libro> topLibros = libros.stream().sorted(Comparator.comparingInt(Libro::getAnioPublicacion))
				.sorted(Comparator.comparingInt(Libro::getContadorPrestamos).reversed())
				.limit(10) //
				.collect(Collectors.toList());

		topLibros.forEach(libro -> System.out.println(libro.getTitulo() + " - " + libro.getContadorPrestamos()
				+ " prestamos, Año: " + libro.getAnioPublicacion()));
	}

	public void imprimirUsuariosConPrestamos() {
		List<Usuario> usuariosConPrestamos = usuarios.stream()
				.filter(usuario -> prestamos.stream().anyMatch(prestamo -> prestamo.getUsuario().equals(usuario))) 
				.sorted(Comparator.comparing(Usuario::getNombre).thenComparing(Usuario::getApellido)) 
				.collect(Collectors.toList());

		usuariosConPrestamos.forEach(usuario -> System.out.println(usuario.getNombre() + " " + usuario.getApellido()
				+ " - " + usuario.getContadorPrestamos() + " prestamos"));
	}

	public Map<Integer, Libro> generarMapaLibrosInfantiles() {
		return libros.stream().filter(libro -> libro.getGenero() == GeneroLibro.INFANTIL) // Filtrar libros infantil
	...
	public void imprimirLibrosInfantil() {
		...
		librosInfantiles.forEach((id, libro) -> System.out.println("ID: " + id + ", Título: " + libro.getTitulo()));
	}
